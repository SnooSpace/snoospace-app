...
import { updateMemberProfile, changeUsername, startEmailChange, verifyEmailChange, fetchInterests } from '../../../api/members';
...
export default function EditProfileScreen({ route, navigation }) {
  ...
  const [pronouns, setPronouns] = useState(profile?.pronouns ? (Array.isArray(profile.pronouns) ? profile.pronouns : [profile.pronouns]) : []);
  ...
  const checkForChanges = () => {
    ...
    const originalPronouns = profile?.pronouns ? (Array.isArray(profile.pronouns) ? profile.pronouns : [profile.pronouns]) : [];
    const originalInterests = profile?.interests || [];
    const originalCity = profile?.city || '';
    const originalLocation = profile?.location || { city: originalCity, state: '', country: '', lat: null, lng: null };
    ...
    const changed = 
      ...
      phone !== originalPhone ||
      email !== originalEmail ||
      JSON.stringify(pronouns.sort()) !== JSON.stringify(originalPronouns.sort()) ||
  ...
  const checkUsernameAvailability = useCallback(async (value) => {
    ...
    try {
      ...
      const result = await apiPost('/username/check', { username: value }, 10000, token);
  ...
  const handleUsernameChange = (value) => {
    ...
    if (sanitized.length >= 3) {
      const timeoutId = setTimeout(() => checkUsernameAvailability(sanitized), 500);
        ...
        <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.section}>
            ...
            <View style={styles.usernameContainer}>
              ...
              />
              {usernameChecking && (
                <ActivityIndicator size="small" color={PRIMARY_COLOR} style={styles.checkIndicator} />
              )}
              {usernameAvailable === true && (
                <Ionicons name="checkmark-circle" size={20} color="#00C851" style={styles.checkIndicator} />
              )}
              {usernameAvailable === false && username !== profile?.username && (
                <Ionicons name="close-circle" size={20} color="#FF4444" style={styles.checkIndicator} />
              )}
            </View>
            {usernameAvailable === false && username !== profile?.username && (
              <Text style={styles.errorText}>Username is already taken or invalid</Text>
            )}
          </View>
