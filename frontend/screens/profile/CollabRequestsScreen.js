 ...
 *
 * Full-featured Collabs hub with three tabs:
 ...
 *
 * Design:
 *   - FONTS.primary (BasicCommercial-Bold) -> Screen titles & card names
 *   - FONTS.semiBold (Manrope-SemiBold)    -> Buttons, badges, tab titles
 *   - FONTS.medium (Manrope-Medium)        -> Metadata, chips, counters
 *   - FONTS.regular (Manrope-Regular)       -> Helper text, descriptions
 *   - Lucide icons only (strokeWidth 1.8-2.2)
...
export default function CollabRequestsScreen({ navigation, route }) {
  ...
  useEffect(() => {
    getActiveAccount()
      ...
      .catch((err) => console.warn('[CollabRequestsScreen] getActiveAccount error:', err));
  }, []);
  ...
  const loadBoard = useCallback(async ({ page = 1, collab_type = boardTypeFilter, append = false } = {}) => {
    ...
    try {
      ...
      console.warn('[CollabRequestsScreen] loadBoard error:', err?.message);
    } finally {
  ...
  const loadPendingApplicationsCount = useCallback(async () => {
    try {
      ...
      console.warn('[CollabRequestsScreen] loadPendingApplicationsCount error:', err?.message);
    }
  }, []);
  ...
  const renderBoardItem = useCallback(({ item }) => {
    const isPoster =
      ...
      (currentUser?.type || 'member') === item.poster_type;
  ...
  }, [currentUser, actionLoading, handleRequestJoin, handleManageApplicants, handlePressProfile]);
      ...
      <SafeAreaView style={styles.container} edges={['top']}>
        ...
        {selectedBoardPost && (
          <CollabRequestSheet
            ...
            onSuccess={() => {
              ...
              showToast(
                'Request sent!',
                `Your request to join "${selectedBoardPost.title}" has been sent.`,
              );
            }}
          />
        )}

        {/* ── Post an Opening Modal ── */}
        <CreateBoardPostModal
          ...
          onSuccess={() => {
            loadBoard({ page: 1, collab_type: boardTypeFilter });
            loadPendingApplicationsCount();
            showToast('Opening posted!', 'Your opening is now live on the Board.');
          }}
        />

        {/* ── My Openings / Manage Applications Modal ── */}
        <MyBoardPostsModal
...
const styles = StyleSheet.create({
  ...
  tabTextActive: { color: TEAL, fontFamily: FONTS.semiBold },
  activeTabIndicator: {
    position: 'absolute',
    bottom: 0,
    height: 2.5,
  ...
  // Board top action row (Manage Applications + Post Opening)
  boardActionRow: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
