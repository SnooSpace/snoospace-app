...
export default function ConnectionsScreen({ navigation }) {
  ...
  const [respondingTo, setRespondingTo] = useState(null); // request id being responded to
  ...
  const handleRespond = useCallback(async (requestId, action) => {
    ...
    try {
      const token = await getAuthToken();
      const result = await apiPost(`/connections/${requestId}/respond`, { action }, 15000, token);

      // Remove from pending list
      setPendingRequests((prev) => prev.filter((r) => r.id !== requestId));

      // If accepted, refresh connections to show the new one
      if (action === "accept") {
        loadData();

        // If there was a conversation created (icebreaker chat), we could navigate to it
        // but for now just reload the list
      }
    } catch (error) {
  ...
  const handleMessage = useCallback(async () => {
    ...
    try {
      const result = await resolveConversation(selectedConnection.member_id, "member");
      ...
      console.warn("[ConnectionsScreen] Failed to resolve conversation:", err);
      // Fallback: navigate with just recipientId
      navigation.navigate("Chat", {
        recipientId: selectedConnection.member_id,
        recipientType: "member",
        recipientName: selectedConnection.member_name,
  ...
  const handleViewProfile = useCallback(() => {
    ...
    navigation.navigate("MemberPublicProfile", {
      memberId: selectedConnection.member_id,
    });
  }, [selectedConnection, navigation]);
            ...
            {pendingRequests.map((request) => (
              <View key={request.id} style={styles.requestCard}>
                ...
                <View style={styles.requestInfo}>
                  ...
                  {request.event_title && (
                    <Text style={styles.requestContext} numberOfLines={1}>
          ...
          {connections.length === 0 && pendingRequests.length === 0 ? (
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptySubtext}>
                Connect with people at events to see them here. Browse profiles in the Discover tab to get started.
              </Text>
            </View>
          ) : (
            connections.map((connection) => (
              ...
              >
                <Image
                  source={{ uri: connection.member_photo }}
                ...
                <View style={styles.connectionInfo}>
                  ...
                  <View style={styles.connectionMeta}>
                    {connection.event_title && (
                      <Text style={styles.connectionEvent} numberOfLines={1}>
                        Met at {connection.event_title}
                      </Text>
                    )}
                    <ActiveBadge
                      lastActiveAt={connection.member_last_active}
                      size="small"
                    />
                  </View>
                </View>
        ...
        {selectedConnection && (
          <View style={sheetStyles.container}>
            ...
            <View style={sheetStyles.profileHeader}>
              <Image
                source={{ uri: selectedConnection.member_photo }}
              ...
              <View style={sheetStyles.profileInfo}>
                ...
                <ActiveBadge
                  lastActiveAt={selectedConnection.member_last_active}
                  size="medium"
                />
              </View>
            </View>
            ...
            {selectedConnection.icebreaker_message && (
              <View style={sheetStyles.icebreakerSection}>
                <Text style={sheetStyles.icebreakerLabel}>Icebreaker</Text>
                <View style={sheetStyles.icebreakerCard}>
                  <Text style={sheetStyles.icebreakerText}>
                    "{selectedConnection.icebreaker_message}"
                  </Text>
                </View>
              </View>
...
// Connection Profile Sheet styles
