...
export default function HostPlanBottomSheet({ isVisible, onClose, onPlanCreated, navigation }) {
  ...
  const dynamicMaxHeight = SCREEN_HEIGHT * 0.85 - (Platform.OS === 'android' ? keyboardHeight : 0);

  const resetState = () => {
    setActivityType('sports'); setCustomLabel(''); setTitle(''); setCostType('free');
    setCostAmount(''); setVisibility('community_members'); setGenderPref('all');
  ...
  const validate = () => {
    ...
    if (activityType === 'other' && !customLabel.trim()) e.customLabel = 'Required for "Other"';
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const body = {
        ...
        custom_activity_label: activityType === 'other' ? customLabel.trim() : undefined,
        title: title.trim(),
        cost_type: costType,
        cost_amount_paise: costAmount ? Math.round(parseFloat(costAmount) * 100) : null,
        visibility,
      ...
      if (err.status === 403 && err.data?.error === 'proof_gate_required') {
        Alert.alert(
          ...
          [
            ...
            {
              text: 'Get verified',
              onPress: () => { onClose(); navigation?.navigate('VerificationSubmit'); },
            },
          ]
        );
      } else {
            ...
            >
              ...
              <View style={styles.chipRow}>
                {ACTIVITIES.map(a => (
                  ...
                  >
                    <Text style={[styles.chipText, activityType === a.key && styles.chipTextActive]}>
                      {a.label}
                    </Text>
                  </TouchableOpacity>
                ))}
              ...
              {errors.customLabel && <Text style={styles.errorText}>{errors.customLabel}</Text>}
              ...
              <View style={styles.chipRow}>
                {COST_OPTS.map(c => (
                  ...
                  >
                    <Text style={[styles.chipText, costType === c.key && styles.chipTextActive]}>
                      {c.label}
                    </Text>
                  </TouchableOpacity>
                ))}
              ...
              <View style={styles.visibilityRow}>
                <TouchableOpacity
                  style={[styles.visCard, visibility === 'community_members' && styles.visCardActive]}
                  onPress={() => setVisibility('community_members')}
                >
                  <Users size={18} color={visibility === 'community_members' ? COLORS.primary : COLORS.textSecondary} strokeWidth={1.8} />
                  <Text style={[styles.visCardTitle, visibility === 'community_members' && { color: COLORS.primary }]}>
                    Community members
                  </Text>
                  <Text style={styles.visCardSub}>People who share a community with you</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={[styles.visCard, visibility === 'everyone' && styles.visCardActive]}
                  onPress={() => setVisibility('everyone')}
                >
                  <Globe size={18} color={visibility === 'everyone' ? COLORS.primary : COLORS.textSecondary} strokeWidth={1.8} />
                  <Text style={[styles.visCardTitle, visibility === 'everyone' && { color: COLORS.primary }]}>
                    Everyone
                  </Text>
                  <Text style={styles.visCardSub}>Visible to all SnooSpace users</Text>
                </TouchableOpacity>
              </View>
              ...
              <View style={styles.chipRow}>
                {GENDER_OPTS.map(g => (
                  ...
                  >
                    <Text style={[styles.chipText, genderPref === g.key && styles.chipTextActive]}>
                      {g.label}
                    </Text>
                  </TouchableOpacity>
                ))}
              ...
              <View style={styles.twoColRow}>
                <View style={{ flex: 1 }}>
                  ...
                  <View style={styles.whenButtonsRow}>
                    <TouchableOpacity
                      style={[styles.input, styles.dateFieldSub, errors.date && styles.inputError]}
                      onPress={() => setDatePickerOpen(true)}
                    >
                      <Calendar size={14} color={COLORS.textMuted} strokeWidth={1.8} />
                      <Text style={[styles.dateFieldText, !selectedDate && { color: COLORS.textMuted }]} numberOfLines={1}>
                        {selectedDate ? selectedDate.toLocaleDateString('en-IN', { day: 'numeric', month: 'short' }) : 'Date'}
                      </Text>
                    </TouchableOpacity>

                    <TouchableOpacity
                      style={[styles.input, styles.dateFieldSub, errors.time && styles.inputError]}
                      onPress={() => setTimePickerOpen(true)}
                    >
                      <Clock size={14} color={COLORS.textMuted} strokeWidth={1.8} />
                      <Text style={[styles.dateFieldText, !selectedTime && { color: COLORS.textMuted }]} numberOfLines={1}>
                        {selectedTime ? selectedTime.toLocaleTimeString('en-IN', { hour: 'numeric', minute: '2-digit', hour12: true }) : 'Time'}
                      </Text>
                    </TouchableOpacity>
                  </View>
