  ...
  (
    {
      ...
      onMediaTypesChange, // Callback to pass media types to parent
      onCropMetadataChange, // Callback to pass crop metadata to parent
      onMutedIndicesChange, // NEW: Callback to pass muted video indices to parent
      initialImages = [],
      initialAspectRatios = [], // Initial aspect ratios
      initialMediaTypes = [], // Initial media types ('image' | 'video')
      caption = "", // Caption for preview
      currentUser = null, // User data for preview
      style,
    ...
    // NEW: Muted videos state - Set of indices that have been muted
    ...
    const handleAddImages = async (
      ...
      // Request media library permission BEFORE opening the picker modal so the
      // system dialog fires cleanly (Android blocks dialogs shown inside modals).
      // Only request 'photo' (+ 'video' if needed) — omitting 'audio' avoids
      // the AndroidManifest declaration error on Android 13+.
      const granularPermissions = allowVideos ? ["photo", "video"] : ["photo"];
      ...
      launchPicker(targetIndex, remainingSlots, isCropEnabled, specificMediaType);
    };
    ...
    // Called when user taps Done in the custom picker
    const handlePickerDone = async (selectedAssets) => {
      setIsPickerVisible(false);
      if (!selectedAssets || selectedAssets.length === 0) return;

      const { targetIndex, isCropEnabled } = pickerConfig;

      try {
        // Map expo-media-library assets to the shape expected by processWithCrop / processWithoutCrop
        const rawAssets = selectedAssets.map((asset) => ({
          uri: asset.uri,
        ...
        console.error("Error processing picked images:", error);
      }
    };
    ...
    // NEW: Toggle mute for a video at a given index
    const toggleVideoMute = (index) => {
      setMutedVideoIndices((prev) => {
        ...
        if (onMutedIndicesChange) {
          onMutedIndicesChange(next);
        }
        return next;
      });
    };
    ...
    const renderImageGrid = () => {
      ...
      // For horizontal tray: simple ScrollView (drag-to-reorder removed)
      if (horizontal) {
        const getThumbDimensions = (index) => {
          ...
          let thumbWidth = thumbHeight * ratio;
        ...
        return (
          ...
          >
            {images.map((imageUri, index) => {
              const { thumbWidth, thumbHeight } = getThumbDimensions(index);
              const metadata = cropMetadata[index];

              return (
                <TouchableOpacity
                  key={`${index}-${originalUris[index] || index}`}
                  activeOpacity={0.9}
                ...
                >
                  ...
                  >
                    ...
                    {/* Muted badge on video thumbnail */}
                    {mediaTypes[index] === "video" && mutedVideoIndices.has(index) && (
            ...
            {images.length < maxImages && (
              <TouchableOpacity
                style={[styles.addMoreButton, { width: 100, height: 120, marginRight: 12 }]}
      ...
      // Non-horizontal (vertical grid) — keep original rendering
      return (
        <ScrollView
          showsHorizontalScrollIndicator={false}
        ...
        >
          ...
          {images.length < maxImages && (
            ...
            >
              ...
              >
                <LinearGradient colors={["#448AFF", "#2962FF"]} style={{ width: 32, height: 32 }} />
              </MaskedView>
            </TouchableOpacity>
          )}
        </ScrollView>
          ...
          >
            ...
            <View style={styles.modalHeader}>
              ...
              <View style={styles.modalActions}>
                {/* Mute Toggle */}
                <TouchableOpacity
                  onPress={() => previewVideoIndex !== null && toggleVideoMute(previewVideoIndex)}
                  style={[
                    styles.modalMuteButton,
                    previewVideoIndex !== null && mutedVideoIndices.has(previewVideoIndex) && styles.modalMuteButtonActive,
                  ]}
                  hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}
                >
                  {previewVideoIndex !== null && mutedVideoIndices.has(previewVideoIndex) ? (
                    <VolumeX size={18} color={COLORS.primary} strokeWidth={2} />
                  ) : (
                    <Volume2 size={18} color={COLORS.textPrimary} strokeWidth={2} />
                  )}
                </TouchableOpacity>
                <TouchableOpacity
