...
const TicketTypesEditor = ({ ticketTypes = [], onChange }) => {
  ...
  const handleSave = () => {
    ...
    const ticketData = {
      ...
      total_quantity: currentTicket.total_quantity ? parseInt(currentTicket.total_quantity) : null,
    ...
    <View style={styles.container}>
      ...
      {ticketTypes.length === 0 && (
        <View style={styles.emptyState}>
          ...
          <Text style={styles.emptySubtext}>Add tickets to enable registrations</Text>
        </View>
      )}
      ...
      {ticketTypes.map((ticket, index) => (
        ...
        >
          <View style={styles.ticketInfo}>
            <View style={styles.ticketHeader}>
              ...
              {ticket.visibility !== 'public' && (
                <View style={styles.visibilityBadge}>
                  <Ionicons 
                    name={ticket.visibility === 'hidden' ? 'eye-off' : 'lock-closed'} 
                    size={12} 
                    color="#666" 
                  />
            ...
            <Text style={styles.ticketPrice}>{formatPrice(ticket.base_price)}</Text>
          ...
          <View style={styles.modalContent}>
            ...
            <ScrollView style={styles.modalBody} showsVerticalScrollIndicator={false}>
              ...
              <TextInput
                ...
                onChangeText={(text) => setCurrentTicket({ ...currentTicket, base_price: text })}
              ...
              <TextInput
                ...
                onChangeText={(text) => setCurrentTicket({ ...currentTicket, total_quantity: text })}
                ...
                {['public', 'hidden', 'invite_only'].map((vis) => (
                  <TouchableOpacity
                    ...
                    style={[
                      ...
                      currentTicket.visibility === vis && styles.visibilityOptionActive,
                    ]}
                    onPress={() => setCurrentTicket({ ...currentTicket, visibility: vis })}
                  >
                    <Ionicons
                      name={
                        vis === 'public' ? 'globe-outline' :
                        vis === 'hidden' ? 'eye-off-outline' : 'lock-closed-outline'
                      }
                      size={18}
                      color={currentTicket.visibility === vis ? COLORS.primary : LIGHT_TEXT_COLOR}
                    />
                    <Text
                      style={[
                        styles.visibilityOptionText,
                        currentTicket.visibility === vis && styles.visibilityOptionTextActive,
                      ]}
                    >
              ...
              {/* Gender Restriction */}
              <Text style={styles.inputLabel}>Gender Restriction</Text>
              <View style={styles.visibilityOptions}>
                {['all', 'Male', 'Female', 'Non-binary'].map((gender) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.visibilityOption,
                      currentTicket.gender_restriction === gender && styles.visibilityOptionActive,
                    ]}
                    onPress={() => setCurrentTicket({ ...currentTicket, gender_restriction: gender })}
                  >
                    <Ionicons
                      name={
                        gender === 'all' ? 'people-outline' :
                        gender === 'Male' ? 'male-outline' :
                        gender === 'Female' ? 'female-outline' : 'transgender-outline'
                      }
                      size={18}
                      color={currentTicket.gender_restriction === gender ? COLORS.primary : LIGHT_TEXT_COLOR}
                    />
                    <Text
                      style={[
                        styles.visibilityOptionText,
                        currentTicket.gender_restriction === gender && styles.visibilityOptionTextActive,
                      ]}
                    >
