...
async function ensureTables(pool) {
  try {
    await pool.query(`
      ...
      EXCEPTION WHEN duplicate_column THEN NULL; END $$;
      -- Attendance confirmation columns
      DO $$ BEGIN
        ALTER TABLE event_registrations ADD COLUMN IF NOT EXISTS attendance_status TEXT;
      EXCEPTION WHEN duplicate_column THEN NULL; END $$;
      DO $$ BEGIN
        ALTER TABLE event_registrations ADD COLUMN IF NOT EXISTS attendance_confirmed_at TIMESTAMPTZ;
      EXCEPTION WHEN duplicate_column THEN NULL; END $$;
      ...
      DO $$ BEGIN
        ALTER TABLE ticket_types ADD COLUMN IF NOT EXISTS gender_restriction TEXT DEFAULT 'all';
      EXCEPTION WHEN duplicate_column THEN NULL; END $$;
      -- Fix any rows where gender_restriction is NULL (shouldn't happen, but safety net)
      UPDATE ticket_types SET gender_restriction = 'all' WHERE gender_restriction IS NULL;
      ...
      -- Add campus_id to members table (for structured college linking)
      DO $$ BEGIN
        ALTER TABLE members ADD COLUMN IF NOT EXISTS campus_id UUID REFERENCES campuses(id);
      EXCEPTION WHEN duplicate_column THEN NULL; END $$;
      -- Show/hide college on profile (default true)
      DO $$ BEGIN
        ALTER TABLE members ADD COLUMN IF NOT EXISTS show_college BOOLEAN DEFAULT true;
      EXCEPTION WHEN duplicate_column THEN NULL; END $$;
      ...
      -- Add community type and college affiliation columns
      DO $$ BEGIN
      ...
      EXCEPTION WHEN duplicate_column THEN NULL; END $$;
      DO $$ BEGIN
        ALTER TABLE communities ADD COLUMN IF NOT EXISTS college_id UUID;
      EXCEPTION WHEN duplicate_column THEN NULL; END $$;
      DO $$ BEGIN
      ...
      EXCEPTION WHEN duplicate_column THEN NULL; END $$;
      -- Default existing community_type NULL rows to 'organization'
      UPDATE communities SET community_type = 'organization', is_sponsor_visible = true WHERE community_type IS NULL;
      ...
      EXCEPTION WHEN OTHERS THEN NULL; END $$;
      -- Add role column for storing head's role (Coordinator, President, etc.)
      DO $$ BEGIN
        ALTER TABLE community_heads ADD COLUMN IF NOT EXISTS role TEXT;
      EXCEPTION WHEN duplicate_column THEN NULL; END $$;
      ...
      EXCEPTION WHEN duplicate_object THEN NULL; END $$;
      -- REMOVED: email_unique_comm constraint for multi-account support
      -- Multiple community accounts can share the same email (Instagram-style)
      DO $$ BEGIN
        ALTER TABLE communities DROP CONSTRAINT IF EXISTS email_unique_comm;
      EXCEPTION WHEN OTHERS THEN NULL; END $$;
      -- REMOVED: sponsor_types_len constraint
      -- Individual Organizers and College-affiliated communities have empty sponsor_types
      -- Only Organization type requires sponsor_types (validated in backend)
      DO $$ BEGIN
