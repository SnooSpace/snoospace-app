async function signup(req, res) {
  try {
    ...
    if (isOrganization || isIndividualOrganizer) {
      ...
      // sponsor_types is optional — empty array means "not looking for sponsors"
    }
    ...
    try {
      ...
      // Prepare sponsor_types - optional for all community types; empty array means opted out
      const finalSponsorTypes = Array.isArray(sponsor_types) ? sponsor_types : [];
      ...
      const communityResult = await client.query(
        `INSERT INTO communities (
          ...
          community_type, college_id, campus_id, college_subtype, club_type, community_theme, is_sponsor_visible
        )
         VALUES ($1,$2,$3,$4,$5,$6::jsonb,$7::jsonb,$8,$9,$10,$11::jsonb,$12,$13,$14,$15,$16,$17,$18,$19)
         RETURNING *`,
        [
          user_id,
          name,
...
async function getProfile(req, res) {
  try {
    ...
    if (community.community_type === 'college_affiliated') {
      try {
        // 1. Try campus_id first (most precise)
        if (community.campus_id) {
          ...
          if (campusResult.rows.length > 0) {
            ...
            collegeInfo = {
              ...
              community_type: community.community_type,
              college_subtype: community.college_subtype,
              club_type: community.club_type,
            };
          }
        }

        // 2. Try college_id if campus lookup failed or campus_id not set
        if (!collegeInfo && community.college_id) {
          const collegeResult = await pool.query(
            `SELECT co.id as college_id, co.name as college_name, co.abbreviation as college_abbreviation,
                    co.logo_url as college_logo_url, co.status as college_status
             FROM colleges co WHERE co.id = $1`,
            [community.college_id]
          );
          if (collegeResult.rows.length > 0) {
            const cr = collegeResult.rows[0];
            collegeInfo = {
              college_id: cr.college_id,
              college_name: cr.college_name,
              college_abbreviation: cr.college_abbreviation,
        ...
        // 3. Fallback: college_id exists in community but not found in colleges table
        //    (can happen with old data from before schema migration was applied)
        //    Build a minimal college_info so the chip still renders.
        if (!collegeInfo && (community.campus_id || community.college_id)) {
          console.warn('[CommunityProfile] College UUID not in DB, using minimal fallback. college_id:', community.college_id);
          collegeInfo = {
            college_id: community.college_id || null,
            college_name: null, // unknown, chip will show generic text
            college_abbreviation: null,
            college_logo_url: null,
            college_status: 'pending', // assume pending for old data
            campus_id: community.campus_id || null,
            campus_name: null,
            campus_city: null,
            community_type: community.community_type,
            college_subtype: community.college_subtype,
        ...
        // 4. Last resort: college_affiliated but no IDs at all — still show chip with subtype info
        if (!collegeInfo) {
          collegeInfo = {
            college_id: null,
            college_name: null,
        ...
        console.error('[CommunityProfile] Failed to fetch college info:', collegeErr);
      }
    }
...
async function patchProfile(req, res) {
  try {
    ...
    if (sponsor_types !== undefined) {
      ...
      // Empty array is valid — means community opted out of sponsoring
      // For non-empty arrays, filter to valid strings only
  ...
  try {
    ...
    if (profile.community_type === 'college_affiliated') {
      try {
        ...
        if (profile.campus_id) {
          ...
          if (campusResult.rows.length > 0) {
            ...
            collegeInfo = {
              ...
              college_subtype: profile.college_subtype,
        ...
        // 2. Try college_id if campus lookup failed or campus_id not set
        if (!collegeInfo && profile.college_id) {
          const collegeResult = await pool.query(
            `SELECT co.id as college_id, co.name as college_name, co.abbreviation as college_abbreviation,
                    co.logo_url as college_logo_url, co.status as college_status
             FROM colleges co WHERE co.id = $1`,
        ...
        // 3. Fallback: college_id exists in community but not found in colleges table
        if (!collegeInfo && (profile.campus_id || profile.college_id)) {
          console.warn('[PublicCommunity] College UUID not in DB, using minimal fallback. college_id:', profile.college_id);
          collegeInfo = {
        ...
        // 4. Last resort: college_affiliated but no IDs — still show chip
        if (!collegeInfo) {
          collegeInfo = {
            college_id: null,
            college_name: null,
        ...
        console.error('[PublicCommunity] Failed to fetch college info:', collegeErr);
      }
    }
