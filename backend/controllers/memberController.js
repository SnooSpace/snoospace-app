...
async function getProfile(req, res) {
  try {
    ...
    const profileData = {
      ...
      campus_id: member.campus_id || null,
      show_college: member.show_college !== false,
      occupation_details: member.occupation_details || null,
      ...
      spotify_connected: !!member.spotify_connected,
      spotify_top_artists:
    ...
    if (member.campus_id) {
      try {
        ...
        console.error('[MemberProfile] Failed to fetch college info:', collegeErr);
      }
    }
    profileData.college_info = collegeInfo;
  ...
  try {
    ...
    if (profile.campus_id && profile.show_college !== false) {
      try {
        ...
        console.error('[PublicMember] Failed to fetch college info:', collegeErr);
      }
    }
...
async function patchProfile(req, res) {
  try {
    ...
    if (req.body.hasOwnProperty('campus_id')) {
      ...
      if (campusId === null) {
        updates.push(`campus_id = NULL`);
      } else if ((typeof campusId === 'string' && campusId.trim().length > 0) ||
                 (typeof campusId === 'number' && !isNaN(campusId))) {
        // Accept both string and numeric IDs
        const campusIdStr = String(campusId).trim();
        updates.push(`campus_id = $${paramIndex++}`);
        values.push(campusIdStr);
      }
    }

    if (req.body.hasOwnProperty('show_college')) {
      updates.push(`show_college = $${paramIndex++}`);
      values.push(req.body.show_college !== false);
    }

    if (spotify_connected !== undefined) {
      updates.push(`spotify_connected = $${paramIndex++}`);
      values.push(!!spotify_connected);
    }

    if (spotify_top_artists !== undefined) {
      if (spotify_top_artists === null) {
        updates.push(`spotify_top_artists = NULL`);
      } else if (Array.isArray(spotify_top_artists)) {
        const sanitized = spotify_top_artists
          .filter(x => typeof x === 'string' && x.trim().length > 0)
          .map(x => x.trim())
          .slice(0, 10);
        updates.push(`spotify_top_artists = $${paramIndex++}::jsonb`);
        values.push(JSON.stringify(sanitized));
      }
    }
...
async function toggleCreatorMode(req, res) {
  ...
  try {
    ...
    // ON disable: mark active non-superseded follower rows as dormant (count drops to 0 via trigger)
    // ON re-enable: restore dormant non-superseded rows (superseded rows stay superseded)
    if (enabled) {
      await client.query(
        `UPDATE creator_follows
         SET is_dormant = false
         WHERE creator_id = $1 AND is_dormant = true AND is_superseded_by_circle = false`,
        [memberId]
      );
    } else {
      await client.query(
