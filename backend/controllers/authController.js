...
// Helper: fetch college_info for a member row that has campus_id
async function fetchCollegeInfoForMember(pool, campusId) {
  if (!campusId) return null;
  try {
    const result = await pool.query(
      `SELECT ca.id as campus_id, ca.campus_name, ca.city as campus_city,
              co.id as college_id, co.name as college_name,
              co.abbreviation as college_abbreviation,
    ...
    console.error('[getUserProfile] fetchCollegeInfo error:', e.message);
  }
  return null;
}
