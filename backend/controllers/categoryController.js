        ...
        let memberQuery = `
          WITH member_list AS (
            SELECT 
              ...
              m.is_verified, m.verification_tier, m.verified_at, m.gender, m.dob,
    ...
    if (type === "member") {
      const result = await pool.query(
        `SELECT 
          ...
          m.created_at, m.instagram_username, m.is_creator_mode_enabled,
          m.creator_follower_count, m.creator_mode_enabled_at,
          m.is_verified, m.verification_tier, m.verified_at, m.occupation, m.education, m.passout_year, m.show_college_on_profile,
          m.circle_count,
          col.name as college_name,
          cam.campus_name as campus_name,
          (SELECT COUNT(*) FROM follows WHERE following_id = m.id AND following_type = 'member') as follower_count,
